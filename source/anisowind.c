...
int 
randwind_thermal_trapping(p, nnscat, in_clump)
  PhotPtr p;
  int *nnscat;
  int in_clump;
{
  ...
  tau_norm = sobolev (one, p->x, -1.0, lin_ptr[p->nres], one->dvds_max, in_clump);
  ...
  *nnscat = *nnscat - 1;
   ...
   while (ztest > z)
  {
    *nnscat = *nnscat + 1; //- JM - see above 
    ...
    dvds = dvwind_ds (p);
    tau = sobolev (one, p->x, -1.0, lin_ptr[p->nres], dvds, in_clump);
